...
export const PrintReportModal: React.FC<PrintReportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div
        className="relative w-full max-w-4xl max-h-[92vh] flex flex-col rounded bg-[#0a0a0a] border border-white/10 shadow-2xl overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Top Bar - Hidden on actual print */}
        <div className="no-print flex items-center justify-between px-6 py-4 border-b border-white/10 bg-black/40">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
              <Printer className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-base font-semibold text-white tracking-tight">Daily Report &amp; Accountability Log</h2>
              <p className="text-xs text-[#8e9299]">
                Print or export executive log for {formattedDate}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 rounded text-xs uppercase tracking-wider font-semibold bg-white/5 hover:bg-white/10 text-[#e5e5e5] border border-white/10 flex items-center gap-1.5 transition-colors"
              title="Download CSV"
            >
            ...
            <button
              onClick={handlePrint}
              className="px-4 py-1.5 rounded text-xs uppercase tracking-widest font-bold bg-emerald-600 hover:bg-emerald-500 text-white shadow-[0_0_15px_rgba(16,185,129,0.3)] flex items-center gap-1.5 transition-colors"
            >
